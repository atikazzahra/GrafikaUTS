...
class Line : public Shape
{
    public:
		...
		void drawCircle(int r, int xc, int yc, int full){
		 ...
		 * Prosedur untuk ngegambar lingkaran Bresenham*/
			...
			while (xn<yn){
				...
				//Sisi kanan atas
				Point(x3+xc,y3+yc);
				Point(-1*y3+xc+0.5*r,-1*x3+yc+0.5*r);
				
				//Sisi kiri atas
				Point(x3*(-1)+xc,y3+yc);
				Point(y3+xc-0.5*r,-1*x3+yc+0.5*r);
				
				if (full==1){
					//sisi kanan bawah
					Point(xn+xc,yn+yc+0.5*r);
					Point(yn+xc,xn+yc+0.5*r);
				
					//sisi kiri bawah
					Point(-1*x3+xc,-1*y3+yc+r);
					Point(y3+xc-0.5*r,x3+yc+0.5*r);
				}
			}
		}
